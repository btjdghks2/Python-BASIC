...
def arith_oper(): # 산술 연산자 연습
    print("===== 산술연산")
    # + - * /
    print(7/3) # 정수를 정수로 나눠도 실수 형태로 표현
    print(6/2)
...
def rel_oper() : # 비교 연산자 연습
    ...
    print("answkdufdml ==", s1== s2)

    # 복합 관계식
    a = 6
    # a가 0보다 크고 a가 10 보다 작은가?
    print(a > 0 and a< 10) #
    print(0 < a and a< 10) #
    print(0< a < 10)
...
def assginment_ex(): # 치환문
    ...
    # 여러 변수를 한꺼번에 할당
    a, b = 3.5, 5.3 # 좌변의 변수의 갯수와 우변의 값의 겟수 일치해야 한다
    print(a,b)
    ...
    # 중요 : 동적 타이핑
    # 파이썬은 변수 선언에 없고, 값이 할당될 때 데이터 타입 결정
    a= 1
    print(a,"is", type(a)) # type함수 -> 데이터 혹은 객체의 데이터 타입을 확인
    a = "Hello Python" # 동적 타이핑
    print(a, "is", type(a))

    # isinstonce(판별할 값 or 객체, 데이터 타입)
    print("a는 str의 객체?", isinstance(a,str))

def logical_oper():
        print("===== 논리 연산")
        # 논리곱(and : 둘다 true일 때만 true)
        # 논리합(or : 둘 중 하나만 true면true)
        # 논리부정(not : true<-> false)
        a, b = 20, 50
        print(not a < b) # a < b 의 논리를 부정
        print(a < b and a != b) # a< b의 논리 값과 != b의 논리값의논리곱
        print(a == b or a != b) # a == b 의 논리값과 a!=의 논리값의 논리합
