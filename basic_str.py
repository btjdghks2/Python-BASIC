...
파이썬은 데어터 처리가 중요한 시대에서
가장 널리 사용되는 언어이다.
    
    """
    print(s4)

    # 여러줄 문자열은 한 줄 주석만 있는 파이썬에서
    # 여러줄 주석을 사용할 경우에도 사용
    # 메서드 정의부 바로 아래 여러 줄 문자열을 입력하면
    # help 명령어를 이용해서 해당 메서드의 주석을 볼수 있다.
...
def string_oper(): # 문자열 연산
    ...
    print("[d:len(str1)] 슬라이싱:", str1[3:len(str1)])
    print("[3:] 슬라이싱", str1[3:]) # 끝경계를 생략 -> 끝까지
...
def transform_mathods():
    ...
    print("TITLE : ", s.title()) # 가사제목 형태로 : 각 단어의 첫 글자를 대문자로 변환
    print("원본:", s) # str 객체 immutoble -> 원본 변경되지는 않음
...
def search_methods():
    ...
    print("3nd Find:", s.find("Like", 20)) # 검색 결과 없으면 -1 반환
...
def modify_replace_methods():
    ...
    s = "--------------Alice and the Heart Queen------------------"
    print("STRIP -:[",s.strip("-"),"]", sep="") # 앞 뒤의 - 문자를 모두 제거
