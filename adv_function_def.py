...
def my_function():
    print("Hello python") # 끝날때 까지 함수 종료 필요 없을 때, return으로 생략 가능

def times(a ,b): # 입력은 인수로 전달
    return a * b # 반환할 값이 있을때는 return으로 반환

def none():
    return # 반환 없이 rethurn으로 변환
...
# return문으로 여러개의 객체(결과)를 반환할 수 있다. -> 튜플로 패킹되어서 반환
...
c, d = swap(10, 20) # unpacking 으로 여러 값을 전달 받을 수 있다.
print(c, d)